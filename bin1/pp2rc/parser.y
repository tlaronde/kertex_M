%token	array_tok begin_tok case_tok const_tok do_tok downto_tok else_tok
	...
	type_id_tok field_id_tok define_tok field_tok
...
PROGRAM:	DEFS
		PROGRAM_HEAD
			{block_level++;
			 fprintf(std, "int argc;\nchar **argv;\n");
			 fprintf(std, 
				"#define fputs_(stream, s)	fputs(s, stream) /* fixwrites */\n");
			 fprintf(std, "#include <stdlib.h> /* exit() */\n");
			 fprintf(std, "#include \"%s\"\n", std_header);}
		...
		P_F_DEC_PART
			{ fprintf(std, "int\nmain(int argc_, char *argv_[])\n{\n");
			  	indent++;
		      	new_line();
				my_output("argc = argc_;");
		      	new_line();
				my_output("argv = argv_;");
		     	new_line();
             }
		begin_tok 
		STAT_LIST
...
/* The following code says: if the bounds are known at translation time
   on an integral type, then we select first the smallest _signed_ 
   type that can handle it, or the corresponding unsigned if lower bound
   is positive. This is so that promotion and conversion made by C89
   (or undue conversion around unsigned integral by compilers) don't 
   wrap around: unsigned char -> unsigned int -> all unsigned int 
   -> signed int truncated (it has happened with Plan9 compiler) leading
   from (ex.) -201 (correct value) to (2^32 - 201) - 2^31 = 2^31 - 201!
   Only char has to be _explicitely_ said to be signed. Others are so
   by default if not unsigned.
 */
				  if (lower_sym == -1 && upper_sym == -1) {
				    	if (lower_bound >= -127 && upper_bound <= 127)
							my_output("signed char");
				    	else if (lower_bound >= 0 && upper_bound <= 255)
							my_output("unsigned char");
	  			    	else if (lower_bound >= -32767 && upper_bound <= 32767)
							my_output("short");
	  			    	else if (lower_bound >= 0 && upper_bound <= 65535)
...
FOR_STATEMENT:		for_tok 
			...
			STATEMENT
				{
				  char *top = strrchr(for_stack, '#'); /* FOR_LIST => !NULL */
				  indent--; new_line();
				  my_output("while"); 
				  my_output("("); 
				  my_output(top+1); 
